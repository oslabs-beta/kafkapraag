...
const ProducersRate = (): React.ReactNode => {
  // const dataPoints = new Array(100).fill({ x: "", y: 0 });
  const dataPoints: Array<{ x: string, y: number, timestamp: number }> = []
  for (let i = 0; i < 100; i++) {
    dataPoints.push({ x: `${i}`, y: 0, timestamp: Date.now() })
  }

  const [ProducersRate, setProducersRate] = useState<Array<{ x: string, y: number, timestamp: number }>>(dataPoints)
